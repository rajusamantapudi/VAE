...
-- original image
  ...
  for i = 1, z_change:size()[1] do
    local temp_z = current_z:clone()
    for j = 1, 5 do
    temp_z[1][j] = z_change[i]
    end
    local f = decoder:forward({temp_z, cur_attr}) 
    local res = f[1][1]:clone()
    res = torch.squeeze(res)
    res:add(1):mul(0.5) 
...
image.save(opts.modelPath .. string.format('/sample-z.jpg'), formatted)
