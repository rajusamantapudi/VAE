...
opts.modelName = string.format('LFW_%s_adam%d_bs%d_zdim%d_wd%g_new_ydim%d', 
...
for t = epoch+1, opts.maxEpoch do
  ...
  for i = 1, N_train, opts.batchSize do 
    ...
    local opfunc = function(x)
      ...
      local f = decoder:forward({out1, out2})
      ...
      local err = decoder:backward({out1, out2}, df_dw)

     enc_sampling:backward(batch_im, err[1])
      ...
      --print(de_dw)
      encoder:get(1):get(1):backward(batch_im,de_dw)
