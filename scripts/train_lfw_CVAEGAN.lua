...
opts = lapp[[
  ...
  --modelString   (default 'arch_CVAE_GAN')        reload pretrained network
...
if opts.modelString == '' then
  ...
  encoder, generator,discriminator = lfwcvae_module.create(opts)
end
...
for i = opts.maxEpoch,1,-opts.saveFreq do
  if paths.filep(opts.modelPath .. string.format('/net-epoch-%d.t7', i)) then
    ...
    discriminator = loader.discriminator
...
vae_gan = nn.Sequential()
vae_gan:add(cvae)
vae_gan:add(discriminator)
parameters, gradients = vae_gan:getParameters()
print(gradients:size()) --  20082888
...
for t = epoch+1, opts.maxEpoch do
  ...
  for i = 1, N_train, opts.batchSize do 
    ...
    local opfunc = function(x)
      ...
      -- add gan : fake image
      local output = discriminator:forward(batch_im)
      local target = torch.Tensor(opts.batchSize,1):fill(1)  -- all of them are fake
      local CEerr = criteriaCE:forward(output, target)
      local df_ = criteriaCE:backward(output, target)
      local df_dis = discriminator:backward(batch_im, df_)
      local df_zeros = torch.Tensor(opts.batchSize,3,64,64):fill(0)
      cvae:backward({{batch_im, batch_attr}, batch_attr}, {df_dis,df_zeros})

      -- input a real image to discriminator
      local output2 = discriminator:forward(f[1])
      local target2 = torch.Tensor(opts.batchSize,1):fill(0)
      local CEerr = criteriaCE:forward(output2, target2)
      --print(discriminator:getParameters())
      local df_ = criteriaCE:backward(output2, target2)
      local df_dis = discriminator:backward(f[1], df_)
      cvae:backward({{batch_im, batch_attr}, batch_attr}, {df_dis, df_zeros})

      -- add to confusioin
      for i=1,opts.batchSize do
          local c
          if output2[i][1] > 0.5 then c = 2 else c = 1 end
            CONFUSION:add(c, target2[i][1]+1)
      end

      for i=1,opts.batchSize do
          local c
          if output[i][1] > 0.5 then c = 2 else c = 1 end
          CONFUSION:add(c, target[i][1]+1)
      end
  ...
  if t % 1 == 0 then
    ...
    local f = generator:forward({batch_z, batch_attr})
