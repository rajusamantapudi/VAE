...
opts.modelName = string.format('LFW_%s_adam%d_bs%d_zdim%d_wd%g_stacked_ydim%d', 
  opts.modelString, opts.adam, opts.batchSize, opts.zdim, opts.weightDecay, opts.ydim)
...
for t = epoch+1, opts.maxEpoch do
  ...
  for i = 1, N_train, opts.batchSize do 
    ...
    for j = i, i+opts.batchSize-1 do
      ...
      batch_im[k] = cur_im
      batch_attr[k] = torch.Tensor(opts.ydim)
      for j=1, opts.ydim do
         batch_attr[k][j] = cur_attr[attr_ixs[j]]
      end
      k = k + 1
    end
    ...
    local opfunc = function(x)
      ...
      out1 = enc_sampling:forward(batch_im) -- z
      out2 = encoder:get(1):get(2):forward(batch_im) -- y
      
      local ff = enc_sampling2:forward({out1, out2}) -- z
      local KLDerr = criterionKLD:forward(enc_sampling2:get(1).output, nil)
      local err_2= criterionKLD:backward(enc_sampling2:get(1).output, nil) -- linearMix has 2 output

      local f = decoder:forward(ff)
      --local f = cvae:forward({batch_im, batch_im})
      local LLerr = criterionLL:forward(f, batch_im)
      
      local df_dw = criterionLL:backward(f, batch_im)  -- table: 32*3*64*64
     -- decoder takes z & y -> image 
     local err = decoder:backward(ff, df_dw)   -- err has same dimension as ff
 
      local err = enc_sampling2:backward({out1, out2},err)
      --encoder:get(1):get(1):backward(batch_im,{-encoder:get(1):get(1).output[1]:clone() ,err[1]})
      encoder:get(1):get(2):backward(batch_im, err[2]) 
      ...
      enc_sampling:get(1):backward(batch_im,de_dw)

      local err_from2 = enc_sampling2:get(1):backward({out1, out2}, err_2)
--      encoder:get(1):get(1):backward(batch_im, {-(encoder:get(1):get(1).output[1]):clone(),err_from2[1]})  -- update z
      encoder:get(1):get(2):backward(batch_im, err_from2[2])
  ...
  for i = 1, N_val, opts.batchSize do
    ...
    for j = i, i+opts.batchSize-1 do
      ...
      batch_im[k] = cur_im
    --  batch_attr[k] = cur_attr[{attr_ixs}]
      batch_attr[k] = torch.Tensor(opts.ydim)
      for j=1, opts.ydim do
         batch_attr[k][j] = cur_attr[attr_ixs[j]]
      end      
k = k + 1
    ...
    cvae:zeroGradParameters()
    --local f = cvae:forward({batch_im, batch_im})
      out1 = enc_sampling:forward(batch_im)
      out2 = encoder:get(1):get(2):forward(batch_im)
    local ff = enc_sampling2:forward({out1, out2})  
    local f = decoder:forward(ff)
     local LLerr = criterionLL:forward(f, batch_im)
    local KLDerr = criterionKLD:forward(enc_sampling:get(1).output, nil)
    local MSEerr = criterionMSE:forward(encoder:get(1):get(2).output, batch_attr)
  ...
  if t % 1 == 0 then
    ...
    batch_im:mul(2):add(-1)
    local batch_attr = encoder:get(1):get(2):forward(batch_im)
    local batch_z = enc_sampling:forward(batch_im)
    local batch_z = enc_sampling2:forward({batch_z, batch_attr})
